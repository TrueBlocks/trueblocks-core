...
os.system("ls -l output-czips/  | sed 's/$/|czip+/'  >>output-tmp/file")
...
os.system("cat output-tmp/file1 | sed 's/.gz//' | sed 's/.bin/.bin|/' | sed 's/.txt/.txt|/' | sed 's/.bloom/.bloom|/' | tr '|' '\t' | sort -u >output-tmp/file")
os.system("cat output-tmp/file  | sed 's/.txt//' | sed 's/.bin//' | sed 's/.bloom//' >output-tmp/file1")
...
os.system("cat output-tmp/file1 | grep bloom | awk '{print $1,$3}' OFS=, | tr ',' '\t' >output-tmp/bloom.file")
...
os.system("cat output-tmp/file1 | grep czip  | awk '{print $1,$3}' OFS=, | tr ',' '\t' >output-tmp/czip.file")
