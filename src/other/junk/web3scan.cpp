...
public:
    ...
    CAccumulator(void) : totBlocks(0)
        { summary.reserve(2000); CDailySummary sum; sum.timestamp = toTimestamp(earliestDate); summary.push_back(sum); }
    void noticeBlock(const CBlock& block) {
        totBlocks++;
        summary.back().nBlocks++;
        timestamp_t ts = toTimestamp(BOD(dateFromTimeStamp(block.timestamp)));
    ...
    void noticeTransaction(const CTransaction& trans) {
        ...
        bool hasCreate = !zeroAddr(trans.receipt.contractAddress);
        if (hasCreate) {
            foundCreate = true;
#ifdef SHOW_CREATES
    ...
    void noticeTrace(const CTrace& trace) {
        ...
        string_q tAddr = substitute(substitute(substitute(trim(trace.Format("[{traceAddress}]"),'\"'), "\n", ""), "\"", "-"), "-,-", "-");
        if (trace.isError()) {
            ...
            if (tAddr.empty())
                tAddr = "-";
#ifdef SHOW_ERRORS
            cerr << trace.Format(padLeft(tAddr,15) + "\t[{BLOCKNUMBER}].[{transactionPosition}]\t[{type}]\t[{error}]") << "\n";
#endif
            foundError = true;
        }
        bool hasCreate = (trace.type == "create" || trace.action.callType == "create") && !zeroAddr(trace.result.address);
        if (hasCreate) {
            summary.back().nTxCreates +=  isFirstTrace;
            summary.back().nTcCreates += !isFirstTrace; //(!isFirstTrace && !zeroAddr(trace.result.address));
//            summary.back().nTcFails   += (!isFirstTrace && zeroAddr(trace.result.address));
            foundCreate = true;
#ifdef SHOW_CREATES
            cerr << "\t" << summary.back().Format("[{nCreates}]") << ".\t"
...
#if 0
cerr << bn << "\t" << tx << "\t" << trace.transactionHash << "\t" << trace.isError() << "\t" "\t" << trace.action.callType << "\t" << trace.type << "\t" << trace.result.address << "\t" << tAddr << "\n";
#endif
...
bool visitBlock(CBlock& block, void *data) {
    ...
    cout << acc->totBlocks << " of " << acc->options.nBlocks << "\r";
