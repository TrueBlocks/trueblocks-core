...
void COptions::readData(void) {
    ...
    while (!str.empty()) {
        ...
        while (!line.empty()) {
            ...
            uint32_t v = (uint32_t)toUnsigned(val);
...
void COptions::writeData(void) const {
    ...
    for (uint32_t row = 0 ; row < ROWS ; row++) {
        for (uint32_t col = 0 ; col < COLS ; col++) {
            transRes += asStringU(transCount[row][col]);
            traceRes += asStringU(traceCount[row][col]);
            if (col < (COLS-1)) {
                transRes += "\t";
                traceRes += "\t";
            }
