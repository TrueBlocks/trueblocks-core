    ...
    switch (tolower(fieldName[0])) {
        case 'b':
            if ( fieldName % "blocks" ) {
                ...
                while (!str.empty()) {
                    blocks1.push_back(str_2_Uint(nextTokenClear(str, ',')));
                }
                return true;
            }
            break;
        case 'c':
            if ( fieldName % "counter" ) { counter2 = str_2_Uint(fieldValue); return true; }
            break;
        default:
            break;
    }
    ...
    switch (tolower(fieldName[0])) {
        case 'b':
            if ( fieldName % "blocks" || fieldName % "blocksCnt" ) {
                size_t cnt = blocks1.size();
                if (endsWith(fieldName, "Cnt"))
                    return uint_2_Str(cnt);
                if (!cnt) return "";
                string_q retS;
                for (size_t i = 0 ; i < cnt ; i++) {
                    retS += uint_2_Str(blocks1[i]);
                    retS += ((i < cnt - 1) ? ",\n" : "\n");
                }
                return retS;
            }
            break;
        case 'c':
            if ( fieldName % "counter" ) return uint_2_Str(counter2);
            break;
    }
...
const string_q CLeaf::getStringAt(const string_q& name, size_t i) const {
    if ( name % "blocks" && i < blocks1.size() )
        return uint_2_Str(blocks1[i]);
    return "";
}
...
//                    << " (" << first << ")"
                    << "\n";
                counter2++;
                blocks1.push_back(str_2_Uint(_value));
            }
            return this;
            ...
            string_q curVal;
            if (!contains(_key) || blocks1.size() == 0) {
                curVal = "";
            } else {
                curVal = uint_2_Str(counter2);
            }
if (::contains(_key, "492cbf1fdc2c4f6cd6"))
    printf("");
            CTreeNode *n = CTreeNode::newBranch(_key, _value, prefixS, curVal);
            for (auto blk : blocks1)
                n->insert(_key, uint_2_Str(blk));
            delete this;
            return n;
        }
    }
