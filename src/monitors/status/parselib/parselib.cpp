...
void parselib_init(QUITHANDLER qh) {
    ...
    CControllerChangedEvent::registerClass();
    ...
    CCreateCloneTokena1c9::registerClass();
...
const CTransaction *promoteToFunc(const CTransaction *p) {
    ...
    if (p && (p->input.length() >= 10 || p->input == "0x")) {
        ...
        if (encoding == func_acceptOwnership) {
            ...
            // 0x5b7b72c1
            CCreateCloneTokena1c9 *a = new CCreateCloneTokena1c9;
            a->CTransaction::operator=(*p);
            a->_parentToken = str_2_Addr(extract(params, 0*64, 64));
            a->_snapshotBlock = str_2_Wei("0x" + extract(params, 1*64, 64));
            a->_tokenName = extract(params, 2*64, 64);
            a->_decimalUnits = (uint32_t)str_2_Uint(extract(params, 3*64, 64));
            a->_tokenSymbol = extract(params, 4*64, 64);
            a->_transfersEnabled = str_2_Int(extract(params, 5*64, 64));
            items[nItems++] = "address";
            items[nItems++] = "uint256";
            items[nItems++] = "string";
            items[nItems++] = "uint8";
            items[nItems++] = "string";
            items[nItems++] = "bool";
            a->articulatedTx = decodeRLP("createCloneTokena1c9", params, nItems, items);
            return a;
            ...
            a->param_0 = str_2_Addr(extract(params, 1*64, 64));
            a->param_1 = str_2_Wei("0x" + extract(params, 2*64, 64));
            items[nItems++] = "address";
            items[nItems++] = "address";
            items[nItems++] = "uint256";
            ...
            a->CTransaction::operator=(*p);
            a->param_0 = str_2_Addr(extract(params, 0*64, 64));
            items[nItems++] = "address";
...
const CLogEntry *promoteToEvent(const CLogEntry *p) {
    ...
    if (nTops > 0) {  // the '0'th topic is the event signature
        ...
        if (topic_2_Str(p->topics[0]) % evt_ClaimedTokens) {
            ...
            a->_token = str_2_Addr(nTops > 1 ? topic_2_Str(p->topics[1]) : "");
            a->_controller = str_2_Addr(nTops > 2 ? topic_2_Str(p->topics[2]) : "");
            ...
            CControllerChangedEvent *a = new CControllerChangedEvent;
            a->CLogEntry::operator=(*p);
            a->_newController = str_2_Addr(nTops > 1 ? topic_2_Str(p->topics[1]) : "");
            items[nItems++] = "address";
            a->articulatedLog = decodeRLP("ControllerChanged", params, nItems, items);
            return a;
