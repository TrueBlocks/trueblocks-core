...
bool CBuy::setValueByName(const string_q& fieldName, const string_q& fieldValue) {
    ...
    switch (tolower(fieldName[0])) {
        case '_':
            ...
            if ( fieldName % "_depositAmount" ) { _depositAmount = str_2_Wei(fieldValue); return true; }
...
void CBuy::registerClass(void) {
    ...
    ADD_FIELD(CBuy, "_reserveToken", T_ADDRESS, ++fieldNum);
    ADD_FIELD(CBuy, "_depositAmount", T_NUMBER, ++fieldNum);
    ADD_FIELD(CBuy, "_minReturn", T_NUMBER, ++fieldNum);
...
string_q CBuy::getValueByName(const string_q& fieldName) const {
    ...
    switch (tolower(fieldName[0])) {
        case '_':
            ...
            if ( fieldName % "_depositAmount" ) return bnu_2_Str(_depositAmount);
