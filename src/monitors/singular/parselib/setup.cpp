...
bool CSetup::setValueByName(const string_q& fieldName, const string_q& fieldValue) {
    ...
    switch (tolower(fieldName[0])) {
        case 's':
            if ( fieldName % "singularDTVCrowdfundingAddress" ) { singularDTVCrowdfundingAddress = str_2_Addr(fieldValue); return true; }
            if ( fieldName % "singularDTVTokenAddress" ) { singularDTVTokenAddress = str_2_Addr(fieldValue); return true; }
            break;
        default:
            break;
    }
...
bool CSetup::Serialize(CArchive& archive) {
    ...
    // EXISTING_CODE
    // EXISTING_CODE
    archive >> singularDTVCrowdfundingAddress;
    archive >> singularDTVTokenAddress;
    finishParse();
    return true;
}
...
void CSetup::registerClass(void) {
    ...
    ADD_FIELD(CSetup, "singularDTVCrowdfundingAddress", T_ADDRESS, ++fieldNum);
    ADD_FIELD(CSetup, "singularDTVTokenAddress", T_ADDRESS, ++fieldNum);
...
string_q CSetup::getValueByName(const string_q& fieldName) const {
    ...
    switch (tolower(fieldName[0])) {
        case 's':
            if ( fieldName % "singularDTVCrowdfundingAddress" ) return addr_2_Str(singularDTVCrowdfundingAddress);
            if ( fieldName % "singularDTVTokenAddress" ) return addr_2_Str(singularDTVTokenAddress);
            break;
    }
