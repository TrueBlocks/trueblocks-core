...
bool CBidSubmissionEvent::setValueByName(const string_q& fieldName, const string_q& fieldValue) {
    ...
    switch (tolower(fieldName[0])) {
        case '_':
            ...
            if ( fieldName % "_amount_with_bonus" ) { _amount_with_bonus = str_2_Wei(fieldValue); return true; }
            if ( fieldName % "_remaining_funds_to_end_auction" ) { _remaining_funds_to_end_auction = str_2_Wei(fieldValue); return true; }
            break;
        default:
            break;
    }
...
bool CBidSubmissionEvent::Serialize(CArchive& archive) {
    ...
    archive >> _sender;
    archive >> _amount;
    archive >> _amount_with_bonus;
    archive >> _remaining_funds_to_end_auction;
    finishParse();
    return true;
}
...
void CBidSubmissionEvent::registerClass(void) {
    ...
    ADD_FIELD(CBidSubmissionEvent, "_sender", T_ADDRESS, ++fieldNum);
    ADD_FIELD(CBidSubmissionEvent, "_amount", T_NUMBER, ++fieldNum);
    ADD_FIELD(CBidSubmissionEvent, "_amount_with_bonus", T_NUMBER, ++fieldNum);
    ADD_FIELD(CBidSubmissionEvent, "_remaining_funds_to_end_auction", T_NUMBER, ++fieldNum);
...
string_q CBidSubmissionEvent::getValueByName(const string_q& fieldName) const {
    ...
    switch (tolower(fieldName[0])) {
        case '_':
            ...
            if ( fieldName % "_amount_with_bonus" ) return bnu_2_Str(_amount_with_bonus);
            if ( fieldName % "_remaining_funds_to_end_auction" ) return bnu_2_Str(_remaining_funds_to_end_auction);
            break;
    }
