...
bool CTransferAnyERC20Token::setValueByName(const string_q& fieldName, const string_q& fieldValue) {
    ...
    switch (tolower(fieldName[0])) {
        case 'a':
            ...
            break;
        case 't':
            if ( fieldName % "tokenAddress" ) { tokenAddress = str_2_Addr(fieldValue); return true; }
            break;
        default:
            break;
    }
...
void CTransferAnyERC20Token::registerClass(void) {
    ...
    ADD_FIELD(CTransferAnyERC20Token, "tokenAddress", T_ADDRESS, ++fieldNum);
    ADD_FIELD(CTransferAnyERC20Token, "amount", T_NUMBER, ++fieldNum);
...
string_q CTransferAnyERC20Token::getValueByName(const string_q& fieldName) const {
    ...
    switch (tolower(fieldName[0])) {
        case 'a':
            ...
            break;
        case 't':
            if ( fieldName % "tokenAddress" ) return addr_2_Str(tokenAddress);
            break;
    }
