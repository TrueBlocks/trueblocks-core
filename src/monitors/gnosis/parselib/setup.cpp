...
bool CSetup::setValueByName(const string_q& fieldName, const string_q& fieldValue) {
    ...
    switch (tolower(fieldName[0])) {
        case '_':
            if ( fieldName % "_gnosisToken" ) { _gnosisToken = str_2_Addr(fieldValue); return true; }
            break;
        default:
            break;
    }
...
void CSetup::registerClass(void) {
    ...
    ADD_FIELD(CSetup, "_gnosisToken", T_ADDRESS, ++fieldNum);
...
string_q CSetup::getValueByName(const string_q& fieldName) const {
    ...
    switch (tolower(fieldName[0])) {
        case '_':
            if ( fieldName % "_gnosisToken" ) return addr_2_Str(_gnosisToken);
            break;
    }
