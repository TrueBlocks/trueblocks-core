...
void COptions::displayTrans(ostream& os, const CTransaction *theTrans) const {
    ...
    CTransaction *promoted = (CTransaction *)promoteToFunc(theTrans);
    ASSERT(promoted);
    ...
    format = substitute(substitute(substitute(format, "{PARSE}", ""), "[{FUNCTION}]", functionStr), "[{FUNC}]", toProper(nextTokenClear(functionStr,'|')));
    ...
    if (json_on) {
        for (auto log : theTrans->receipt.logs) {
            CLogEntry *promotedLog = (CLogEntry *)promoteToEvent(&log);
    ...
    if (logs_on) {
        for (uint32_t i = 0 ; i < theTrans->receipt.logs.size() ; i++) {
            ...
            // Display it.
            string_q eventType = extract(string_q(promotedLog->getRuntimeClass()->m_ClassName), 1);
            string_q evtStr = substitute(substitute(promotedLog->toJson1(), "{","{ \"type\": \"" + eventType + "\" "), "}", " }");
            if (!isJson) {
    ...
    if (parse_on || wantsParse) {
        string_q parsed = substitute(substitute(substitute(substitute(promoted->Format("\r\n[{PARSED}]\r\n"), " ", ""), ",", ", "), "{", "{ "), "}", " }");
