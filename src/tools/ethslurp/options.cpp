...
//#define NEW_CODE
#ifdef NEW_CODE
    COption2("addrs", "", "flag", true, true, "one or more addresses to slurp"),
    COption2("blocks", "", "<range>", false, true, "an optional range of blocks to slurp"),
    COption2("type", "", "<tx_type>", false, true, "extract either [ ext | int | token | miner | all ] type of transactions"),
    COption2("fmt", "", "<str>", false, true, "pretty print, optionally add ':txt,' ':csv,' or ':html'"),
    COption2("blocks", "", "<range>", false, true, "export records in block range (:0[:max])"),
    COption2("silent", "", "flag", false, true, "Run silently (only freshen the data, do not display it)"),
    COption2("", "", "", false, true, "Fetches data from EtherScan for an arbitrary address. Formats the output to your specification."),
#else // NEW_CODE
