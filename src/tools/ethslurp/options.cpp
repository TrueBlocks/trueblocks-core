...
// BEG_CODE_OPTIONS
    COption2("addr_list", "", "list<addr>", OPT_REQUIRED | OPT_POSITIONAL, "one or more addresses to slurp"),
    COption2("block_list", "", "list<block>", OPT_POSITIONAL, "an optional range of blocks to slurp"),
    COption2("type", "t", "enum[ext|int|token|miner|all]", OPT_FLAG, "extract either [ext|int|token|miner|all] type of transactions"),
    COption2("blocks", "b", "<range>", OPT_FLAG, "export records in block range (:0[:max])"),
    COption2("silent", "s", "", OPT_SWITCH, "Run silently (only freshen the data, do not display it)"),
    COption2("fmt", "x", "enum[none|json|txt|csv|api]", OPT_FLAG, "pretty print, optionally add ':txt,' ':csv,' or ':html'"),
