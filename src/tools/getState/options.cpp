...
//#define NEW_CODE
#ifdef NEW_CODE
    COption2("addr_list", "", "flag", true, true, "one or more addresses (0x...) from which to retrieve balances"),
    COption2("block_list", "", "flag", true, true, "an optional list of one or more blocks at which to report balances, defaults to 'latest'"),
    COption2("mode", "", "<val>", false, true, "control which state to export. One of [none|some|all|balance|nonce|code|storage|deployed|accttype]"),
    COption2("nozero", "", "flag", false, true, "suppress the display of zero balance accounts"),
    COption2("changes", "", "flag", false, true, "only report a balance when it changes from one block to the next"),
    COption2("noHeader", "", "flag", false, false, "hide the header in txt and csv mode"),
    COption2("fmt", "", "<fmt>", false, false, "export format (one of [none|json|txt|csv|api])"),
    COption2("", "", "", false, true, "Retrieve the balance (in wei) for one or more addresses at the given block(s)."),
#else // NEW_CODE
