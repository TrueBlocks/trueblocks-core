...
bool COptions::parseArguments(string_q& commandIn) {
    ...
    if ((fix || check || count) && !timestamps)
        return usage(usageErrs[ERR_ONLYTIMESTAMP]);
...
COptions::COptions(void) {
    ...
    usageErrs[ERR_ONLYTIMESTAMP] =
        "The --check, --fix, and --count options are only available with the --timestamps option.";  // NOLINT
    // END_ERROR_STRINGS
