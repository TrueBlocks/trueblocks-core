...
| -------: | :------- | :------- |
|  | num | which block (or blocks if more than one) to retrieve (or use [start-stop) for range) |
| -k | --check | pull block(s) using both cache and raw, compare results, report differences (should be none) |
| -s | --source val | either :(c)ache or :(r)aw, source for data retrieval. (shortcuts -c = qblocks, -r = node) |
| -f | --fields val | either :(a)ll, (m)ini, (c)ache or :(r)aw; which fields to include in output (all is default) |
| -p | --parity | mimic parity output using quickBlocks (i.e. quoted hexadecimal for numbers) |
| -t | --terse | if source is raw, retrieve transaction hashes instead of full transactions |
