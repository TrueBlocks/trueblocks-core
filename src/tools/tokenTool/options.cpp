...
bool COptions::parseArguments(string_q& command) {
    ...
    if (token.empty())
        return usage("Please supply a single ERC20 token address. Quitting...");

    // We need a locally running node
    if (!isNodeRunning())
        return usage("This command needs a running Ethereum node. Quitting...");

#ifndef DEBUGGING
    // We need a locally running >> archive << node
    if (!nodeHasBalances())
        return usage("This command only runs if the node has balances. Quitting...");
#endif
