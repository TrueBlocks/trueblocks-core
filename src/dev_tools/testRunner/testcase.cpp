...
void establishTestMonitors(void) {
    if (folderExists(chainConfigs_mocked + "monitors/"))
        return;

    string_q gzipFile = chainConfigs_mocked + "monitors.tar.gz";
    ...
    string_q cmd = substitute(STR_UNZIP_CMD, "[{PATH}]", chainConfigs_mocked);
    // LOG_INFO(cmd);
    // clang-format off
    ...
    for (auto cmd : commands) {
        ...
        if (startsWith(options, match)) {
            ...
            if (match == "RESET") {
                ...
                cleanFolder(chainConfigs_mocked + "unchained");
                options = "";
                if (debug)
                    os << "Cleanup" << endl;
