    ...
    for (auto ep : endpointMap) {
        ...
        for (auto cmd : cmds) {
            ...
            for (auto param : params) {
                ...
                replace(yp, "[{IN}]", (param.option_type == "positional" ? "query" : "query"));
            ...
            if (descr.size()) {
                goCallStream << "\tCallOne(w, r, \"" << descr[0].tool << "\", \"" << cmd << "\")" << endl;
            } else if (route == "AccountsTags" || route == "AccountsEntities") {
                goCallStream << "\tCallOne(w, r, \"ethNames\", \"" << cmd << "\")" << endl;
            } else {
                goCallStream << "\tCallOneExtra(w, r, \"chifra\", \"" << cmd << "\", \"" << cmd << "\")" << endl;
            }
            goCallStream << "}" << endl;
