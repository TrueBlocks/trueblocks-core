...
bool COptions::handle_generate(CToml& toml, const CClassDefinition& classDef, const string_q& ns) {
    ...
    for (auto fld : fieldArray) {
        ...
        // noWrite-min means that a field is part of the object's data (such as CReceipt::blockNumber) but should not be part
        // of the 'class' proper -- i.e. it gets its value from its containing class (in this case the block it belongs to).
        if (!fld.noWrite_min) {
            get_case_stream << fld.Format(caseFmt);
            declare_stream  << convertTypes(fld.Format(declareFmt)) << endl;
            copy_stream     << substitute(substitute(fld.Format(copyFmt), "++SHORT++", "[{SHORT}]"), "++CLASS++", "[{CLASS_NAME}]");
            defaults_stream << fld.Format(setFmt);
            clear_stream    << (fld.isPointer ? fld.Format(ptrClearFmt) : "");
            ar_read_stream  << ((fld.noWrite ? "//" : "") + fld.Format(fld.isPointer ? STR_PRTREADFMT  : STR_READFMT));
            ar_write_stream << ((fld.noWrite ? "//" : "") + fld.Format(fld.isPointer ? STR_PTRWRITEFMT : STR_WRITEFMT));
    ...
    //------------------------------------------------------------------------------------------------
    display_str = (display_str.empty()
                   ? "\"\""
                   : "\n\"[{" + substitute(substitute(trim(toUpper(display_str)), " ", ""), ",", "}][PTAB]\"\n\"[{") + "}]\"");
    ...
    // writeTheCode returns true or false depending on if it WOULD HAVE written the file. If 'test' is true, it doesn't actually write the file
            ...
            while (!fields.empty()) {
                ...
                if (tolower(field12[0]) == ch) {
                    ...
                    if (contains(type, "List") || isPointer) {
                        ...
                        caseCodeOut += " return bool_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "sbool") {
                        caseCodeOut += " return bool_2_Str_t([{PTR}]" + field12 + ");";

                    } else if (type == "bloom") {
                        caseCodeOut += " return bloom_2_Bytes([{PTR}]" + field12 + ");";

                    } else if (type == "wei") {
                        caseCodeOut += " return wei_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "gas") {
                        caseCodeOut += " return gas_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "timestamp") {
                        caseCodeOut += " return ts_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "addr" || type == "address") {
                        caseCodeOut += " return addr_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "hash") {
                        caseCodeOut += " return hash_2_Str([{PTR}]" + field12 + ");";

                    } else if (startsWith(type, "bytes")) {
                        caseCodeOut += " return [{PTR}]" + field12 + ";";

                    } else if (type == "uint8" || type == "uint16" || type == "uint32" || type == "uint64") {
                        caseCodeOut += " return uint_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "blknum") {
                        caseCodeOut += " return uint_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "uint256") {
                        caseCodeOut += " return bnu_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "int8" || type == "int16" || type == "int32" || type == "int64") {
                        caseCodeOut += " return int_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "int256") {
                        caseCodeOut += " return bni_2_Str([{PTR}]" + field12 + ");";

                    } else if (type == "double") {
                        caseCodeOut += " return double_2_Str([{PTR}]" + field12 + ", 5);";

                    } else if (endsWith(type,"_e")) {
                        caseCodeOut +=  " return uint_2_Str(" + field12 + ");";
            ...
            while (!fields.empty()) {
                ...
                if (tolower(field12[0]) == ch) {
                    ...
                    if (contains(type, "List") || isPointer) {
                        ...
                        caseCodeOut +=  " { " + field12 + " = str_2_Bool(fieldValue); return true; }";

                    } else if (type == "sbool") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Bool(fieldValue); return true; }";

                    } else if (type == "bloom") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Bloom(fieldValue); return true; }";

                    } else if (type == "wei") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Wei(fieldValue); return true; }";

                    } else if (type == "gas") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Gas(fieldValue); return true; }";

                    } else if (type == "timestamp") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Ts(fieldValue); return true; }";

                    } else if (type == "addr" || type == "address") {
                        caseCodeOut += " { " + field12 + " = str_2_Addr(fieldValue); return true; }";

                    } else if (type == "hash") {
                        caseCodeOut += " { " + field12 + " = str_2_Hash(fieldValue); return true; }";

                    } else if (startsWith(type, "bytes")) {
                        caseCodeOut += " { " + field12 + " = toLower(fieldValue); return true; }";

                    } else if (type == "int8" || type == "int16" || type == "int32") {
                        caseCodeOut +=  " { " + field12 + " = (int32_t)str_2_Uint(fieldValue); return true; }";

                    } else if (type == "int64") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Int(fieldValue); return true; }";

                    } else if (type == "int256") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Wei(fieldValue); return true; }";

                    } else if (type == "uint8" || type == "uint16" || type == "uint32") {
                        caseCodeOut +=  " { " + field12 + " = (uint32_t)str_2_Uint(fieldValue); return true; }";

                    } else if (type == "uint64") {
                        caseCodeOut +=  " { " + field12 + " = str_2_Uint(fieldValue); return true; }";
                        ...
                        caseCodeOut +=  " { " + field12 + " = str_2_Double(fieldValue); return true; }";

                    } else if (endsWith(type,"_e")) {
                        caseCodeOut +=  " { " + field12 + " = str_2_Enum(" + type + ",fieldValue); return true; }";
                        ...
                        caseCodeOut +=  " { return " + field12 + ".parseJson3(fieldValue); }";

                    } else {
                        caseCodeOut +=  " { " + field12 + " = fieldValue; return true; }";
                    }
...
//------------------------------------------------------------------------------------------------------------
const char* STR_READFMT =
"`archive >> [{NAME}];\n";

//------------------------------------------------------------------------------------------------------------
const char* STR_WRITEFMT =
"`archive << [{NAME}];\n";
//------------------------------------------------------------------------------------------------------------
