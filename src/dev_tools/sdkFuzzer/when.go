...
func DoWhen() {
	...
	if when, _, err := opts.When(); err != nil {
		...
		if err := SaveAndClean[types.NamedBlock]("sdkFuzzer/when.json", when, &opts, func() error {
			_, _, err := opts.When()
			return err
		}); err != nil {
			logger.Error(err)
	...
	if whenTimestamps, _, err := opts.WhenTimestamps(); err != nil {
		...
		if err := SaveAndClean[types.Timestamp]("sdkFuzzer/whenTimestamps.json", whenTimestamps, &opts, func() error {
	...
	if whenCount, _, err := opts.WhenCount(); err != nil {
		...
		if err := SaveAndClean[types.TimestampCount]("sdkFuzzer/whenCount.json", whenCount, &opts, func() error {
