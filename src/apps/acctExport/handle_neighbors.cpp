...
//     for (size_t i = 0; i < trans.receipt.lo gs.size(); i++) {
//         const CLog* l = &trans.receipt.log s[i];
...
//     search.blk = app.blk;
//     search.tx = app.txid;
//     CReverseAppMapEntry* found = (CReverseAppMapEntry*)bsearch(&search, theIndex->reverseAppMap,
//     theIndex->header.nApps,
...
// //-----------------------------------------------------------------------
// // For nei ghbors we handle the entire production of data in the _Pre
// // because what we actually want to do is scan across the index chunks
// bool neighbor s_Pre(CTraverser* trav, void* data) {
//     COptions* opt = reinterpret_cast<COptions*>(data);
...
// //     Note that we do all the processing in the 'neighbo rs_Pre' function
// // }

// //-----------------------------------------------------------------------
// size_t neighbo rs_Count(CTraverser* trav, void* data) {
//     COptions* opt = reinterpret_cast<COptions*>(data);
