...
bool isInteresting(const COptions *opts, const CTransaction *trans, CAddrAccumulator *acc) {
    ...
    for (uint32_t i = 0 ; i < opts->watches.size() ; i++) {
        ...
        // Note: we don't need to look at traces because this is either a regular transaction
        // or a trace pretending to be a transacation. In either case, we only look at the top level
        block.forEveryUniqueAddress(addAddress, shouldFilter, acc);
    }
