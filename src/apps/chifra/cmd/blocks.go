...
One of [ from | to | reward ]`)
	...
	blocksCmd.Flags().BoolVarP(&blocksPkg.GetOptions().CacheTxs, "cache_txs", "", false, "force a write of the block transactions to the cache (hidden)")
	blocksCmd.Flags().BoolVarP(&blocksPkg.GetOptions().CacheTraces, "cache_traces", "", false, "force a write of the block transaction traces to the cache (hidden)")
