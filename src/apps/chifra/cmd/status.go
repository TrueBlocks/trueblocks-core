...
One or more of [ blocks | txs | traces | slurps | prices | all ]`)
	...
	statusCmd.Flags().StringSliceVarP(&statusPkg.GetOptions().Migrate, "migrate", "m", nil, `either effectuate or test to see if a migration is necessary (hidden)
One or more of [ test | abi_cache | block_cache | tx_cache | trace_cache | recon_cache | name_cache | slurp_cache | all ]`)
	statusCmd.Flags().BoolVarP(&statusPkg.GetOptions().GetConfig, "get_config", "g", false, "returns JSON data of the editable configuration file items (hidden)")
	statusCmd.Flags().BoolVarP(&statusPkg.GetOptions().SetConfig, "set_config", "s", false, "accepts JSON in an env variable and writes it to configuration files (hidden)")
