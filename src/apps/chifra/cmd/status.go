...
One or more of [ test | abi_cache | block_cache | tx_cache | trace_cache | recon_cache | name_cache | slurp_cache | all ]`)
	...
	statusCmd.Flags().Uint64VarP(&statusPkg.Options.FirstBlock, "first_block", "S", 0, "first block to process (inclusive -- testing only) (hidden)")
	statusCmd.Flags().Uint64VarP(&statusPkg.Options.LastBlock, "last_block", "E", 0, "last block to process (inclusive -- testing only) (hidden)")
