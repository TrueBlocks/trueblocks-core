...
One of [ toggle | run | restart | pause | quit ]`)
	...
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.Options.AppsPerChunk, "apps_per_chunk", "", 2000000, "the number of appearances to build into a chunk before consolidating it (hidden)")
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.Options.UnripeDist, "unripe_dist", "", 28, "the distance (in blocks) from the front of the chain under which (inclusive) a block is considered unripe (hidden)")
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.Options.SnapToGrid, "snap_to_grid", "", 100000, "an override to apps_per_chunk to snap-to-grid at every modulo of this value, this allows easier corrections to the index (hidden)")
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.Options.FirstSnap, "first_snap", "", 2250000, "the first block at which snap_to_grid is enabled (hidden)")
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.Options.NTestRuns, "n_test_runs", "", 0, "in live testing mode, the number of test runs to process before quitting (hidden)")
