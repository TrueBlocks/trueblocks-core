...
func init() {
	...
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.GetOptions().Settings.Apps_per_chunk, "apps_per_chunk", "", 2000000, "the number of appearances to build into a chunk before consolidating it (hidden)")
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.GetOptions().Settings.Snap_to_grid, "snap_to_grid", "", 250000, "an override to apps_per_chunk to snap-to-grid at every modulo of this value, this allows easier corrections to the index (hidden)")
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.GetOptions().Settings.First_snap, "first_snap", "", 2000000, "the first block at which snap_to_grid is enabled (hidden)")
