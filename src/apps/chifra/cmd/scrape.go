...
func init() {
	...
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.GetOptions().Settings.Apps_per_chunk, "apps_per_chunk", "", 500000, "the number of appearances to build into a chunk before consolidating it (hidden)")
	...
	scrapeCmd.Flags().Uint64VarP(&scrapePkg.GetOptions().Settings.First_snap, "first_snap", "", 500000, "the first block at which snap_to_grid is enabled (hidden)")
