...
func init() {
	...
	exportCmd.Flags().BoolVarP(&exportPkg.GetOptions().Transfers, "transfers", "X", false, `export only eth or token transfers`)
	exportCmd.Flags().BoolVarP(&exportPkg.GetOptions().Assets, "assets", "s", false, `list all assets (with names) that appear in any transfer`)
	...
	exportCmd.Flags().StringSliceVarP(&exportPkg.GetOptions().Asset, "asset", "P", nil, `export transfers, balances, or statements only for this asset`)
	exportCmd.Flags().StringVarP(&exportPkg.GetOptions().Flow, "flow", "f", "", `export transfers, balances, or statements with incoming, outgoing, or zero value
One of [ in | out | zero ]`)
	...
	exportCmd.Flags().BoolVarP(&exportPkg.GetOptions().Accounting, "accounting", "C", false, `deprecated option, you may simply remove it`)
