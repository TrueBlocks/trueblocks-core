...
//#define NEW_CODE
#ifdef NEW_CODE
    COption2("command", "", "flag", true, true, "one of [ leech | scrape | daemon | list | export | balances | stats | ls | rm | accounts | config | slurp | quotes | data | blocks | trans | receipts | logs | traces | state | abi ]"),
    COption2("sleep", "", "<num>", false, true, "for the 'scrape' and 'daemon' commands, the number of seconds chifra should sleep between runs (default 0)"),
    COption2("", "", "", false, true, "Create a TrueBlocks monitor configuration."),
#else // NEW_CODE
