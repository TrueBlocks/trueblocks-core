...
func (l *Ledger) SetContexts(chain string, apps []types.SimpleAppearance, forceSingle bool) error {
	for i := 0; i < len(apps); i++ {
		...
		if forceSingle {
			l.Contexts[key].ReconType = (types.First | types.SameSame | types.Last)
		}
	}
	l.debugContext()
	return nil
