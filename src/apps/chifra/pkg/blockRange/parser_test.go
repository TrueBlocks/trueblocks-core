...
func TestParseBlockNums(t *testing.T) {
	...
	if out.Points[0].Number != 10 {
		t.Error("Mismatched start block:", out.Points[0].Number)
	}

	if out.Points[1].Number != 1000 {
		t.Error("Mismatched end block:", out.Points[1].Number)
	}
...
func TestParseDateStart(t *testing.T) {
	...
	if out.Points[1].Number != 1000 {
		t.Error("Mismatched end date:", out.Points[1].Number)
	}
