...
func ReadChunkHeader(fileName string, checkHash bool) (header indexHeader, err error) {
	...
	hasMagicHash := headerHash == config.HeaderMagicHash
	if !hasMagicHash {
		return header, fmt.Errorf("header has incorrect hash in %s, expected %s, got %s", fileName, config.HeaderMagicHash, headerHash)
	}
