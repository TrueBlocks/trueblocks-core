...
func TestArticulateArgumentsMixedIndexed(t *testing.T) {
	abi, err := abi.JSON(strings.NewReader(encRegistrar))
	if err != nil {
		t.Fatal(err)
	}
...
func TestArticulateArgumentsComplex(t *testing.T) {
	...
	if value := first.Value; value != `[{"owner":"0x66215d23b8a247c80c2d1b7bef4befc2ab384bce","number":0}]` {
