...
func (options *Options) GetCountUnclesInBlock(bn uint64) (uint64, error) {
	method := "eth_getUncleCountByBlockNumber"
	params := rpc.Params{fmt.Sprintf("0x%x", bn)}

	if count, err := rpc.Query[string](options.Chain, method, params); err != nil {
		return 0, err
	} else {
...
func (options *Options) GetUncleHashesByNumber(bn uint64) ([]base.Hash, error) {
	if count, err := options.GetCountUnclesInBlock(bn); err != nil {
		return nil, err
	} else if count > 0 {
		...
		for i := uint64(0); i < count; i++ {
			...
			if rawUncle, err := rpc.Query[types.RawBlock](options.Chain, method, params); err != nil {
				return ret, err
			} else {
...
func (options *Options) GetUnclesByNumber(bn uint64) ([]types.SimpleBlock[types.SimpleTransaction], error) {
	if count, err := options.GetCountUnclesInBlock(bn); err != nil {
		return nil, err
	} else if count > 0 {
