...
func Test_Client(t *testing.T) {
	...
	chainId, networkId, _ := rpcOptions.GetClientIDs(chain)
	if chainId != networkId || chainId != 1 {
		t.Error("provider chain id is 1")
	}

	ts := rpcOptions.GetBlockTimestamp(chain, utils.PointerOf(uint64(1)))
	blockOneTimestamp := int64(1438269988)
	if ts != blockOneTimestamp {
		t.Error("timestamp for block 1 is not correct")
	}

	_, err := rpcOptions.GetTransactionHashFromHashStr(chain, "0x730724cb08a6eb17bf6b3296359d261570d343ea7944a17a9d7287d77900db08")
	if err != nil {
		t.Error("couldn't get known transaction hash from tx hash")
	}

	_, err = rpcOptions.GetTransactionHashByHashAndID(chain, "0x0b4c6fb75ded4b90218cf0346b0885e442878f104e1b60bf75d5b6860eeacd53", 0)
	if err != nil {
		t.Error("couldn't get known transaction hash from block hash and tx id")
	}

	_, err = rpcOptions.GetBlockHashFromHashStr(chain, "0x0b4c6fb75ded4b90218cf0346b0885e442878f104e1b60bf75d5b6860eeacd53")
	if err != nil {
...
func Test_TxFromNumberAndId(t *testing.T) {
	...
	txId := uint64(0)
	_, err := rpcOptions.GetTransactionByNumberAndID("mainnet", uint64(1424623), txId)
	if err != nil {
		t.Fatal(err)
	}
}
