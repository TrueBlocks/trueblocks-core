...
func Test_Client(t *testing.T) {
	...
	chainId, networkId, _ := conn.GetClientIDs(chain)
	if chainId != networkId || chainId != 1 {
		t.Error("provider chain id is 1")
	}

	ts := conn.GetBlockTimestamp(chain, utils.PointerOf(uint64(1)))
	...
	_, err = conn.GetTransactionHashByHashAndID(chain, "0x0b4c6fb75ded4b90218cf0346b0885e442878f104e1b60bf75d5b6860eeacd53", 0)
	if err != nil {
		t.Error("couldn't get known transaction hash from block hash and tx id")
	}

	_, err = conn.GetBlockHashFromHashStr(chain, "0x0b4c6fb75ded4b90218cf0346b0885e442878f104e1b60bf75d5b6860eeacd53")
	if err != nil {
		t.Error("couldn't get known block hash from block hash")
	}

	_, err = conn.GetBlockHashByNumber(chain, 12)
	if err != nil {
...
func Test_TxFromNumberAndId(t *testing.T) {
	...
	txId := uint64(0)
	_, err := conn.GetTransactionByNumberAndID(chain, uint64(1424623), txId)
	if err != nil {
		t.Fatal(err)
	}
}
