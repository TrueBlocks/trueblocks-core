	...
	if rawTraces, err := query.Query[[]types.RawTrace](conn.Chain, method, params); err != nil {
		...
		for _, rawTrace := range *rawTraces {
			traceAction := types.SimpleTraceAction{
				...
				Balance:        *base.NewMyWei(0).SetUint64(utils.MustParseUint(rawTrace.Action.Balance)),
				...
				Value:          *base.NewMyWei(0).SetUint64(utils.MustParseUint(rawTrace.Action.Value)),
			}
