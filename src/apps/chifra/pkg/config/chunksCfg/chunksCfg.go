...
type ChunksSettings struct {
	...
	Estuary_api_key       string  `json:"estuaryApiKey,omitempty"`
...
func (s *ChunksSettings) isDefault(chain, fldName string) bool {
	...
	case "Estuary_api_key":
		return s.Estuary_api_key == def.Estuary_api_key
	case "Sleep":
...
func (s *ChunksSettings) TestLog(chain string, test bool) {
	if !test {
		...
		logger.TestLog(!s.isDefault(chain, "Estuary_api_key"), "Estuary_api_key: ", s.Estuary_api_key)
...
func (base *ChunksSettings) overlay(chain string, overlay ChunksSettings) {
	...
	if !overlay.isDefault(chain, "Estuary_api_key") && overlay.Estuary_api_key != "" {
		base.Estuary_api_key = overlay.Estuary_api_key
	}
...
func GetPinningKeys(chain string) (string, string, string) {
	...
	return s.Pinata_api_key, s.Pinata_secret_api_key, s.Estuary_api_key
}
