...
static const COption params[] = {
    ...
    COption("ui_help", "", "<string>", OPT_HIDDEN | OPT_FLAG, "internal use only"),
...
extern map<string, string> uiHelpMap;
...
bool COptions::parseArguments(string_q& command) {
    ...
    for (auto arg : arguments) {
        ...
        } else if (startsWith(arg, "--ui_help:")) {
            ui_help = substitute(substitute(arg, "-:", ""), "--ui_help:", "");
        } else if (arg == "--ui_help") {
            return flag_required("ui_help");
    ...
    LOG_TEST("ui_help", ui_help, (ui_help == ""));
    // END_DEBUG_DISPLAY

    if (!ui_help.empty()) {
        if (uiHelpMap[ui_help].empty())
            return usage("Invalid ui help location: " + ui_help);
        string_q val = uiHelpMap[ui_help];
        cout << "{ \"data\": [{\"helpText\": \"" << val << "\", \"helpLink\": \"\" }]}" << endl;
        return false;
    }
