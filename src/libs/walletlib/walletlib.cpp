...
//-----------------------------------------------------------------------
const CTransaction_Ex *promoteToWallet(const CTransaction *p) {

    CTransaction_Ex item(p);
    if (p && (p->input.length() >= 10 || p->input == "0x")) {
        ...
        if (encoding == func_addOwner_qb) {
            ...
            a->CTransaction_Ex::operator=(item);
...
//-----------------------------------------------------------------------
const CLogEntry_Ex *promoteToWalletEvent(const CLogEntry *p) {
