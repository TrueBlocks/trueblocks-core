...
namespace qblocks {
    ...
    MYSQL *init_sqldatabase(const SFString& h, const SFString& u, const SFString& p) {
        // connect to mysql server at socket level
        MYSQL *conn = mysql_init(NULL);
        if (!mysql_real_connect(conn, h.c_str(), u.c_str(), p.c_str(), NULL, 0, NULL, 0)) {
            cerr << "Unable to connect to mysql. Error: " << mysql_error(conn) << "\n";
            return NULL;
        }
        cout << "Connected with mysql at " <<   host  << "\n";
        return conn; // caller responsible to clean it up
    }
/*
#include <iostream>
#include <string>
