...
static void ethash_keccakf1600(uint64_t state[25]) {
    int round;
...
static void keccak(uint64_t* out, size_t bits, const uint8_t* data, size_t size) {
...
inline string_q to_hex(const Hash& h) {
...
string_q keccak256(const string& strIn) {
    string_q str = startsWith(strIn, "0x") ? hex_2_Str(strIn) : strIn;
    const uint8_t* const data = reinterpret_cast<const uint8_t*>(str.c_str());
    union ethash_hash256 ret;
    keccak(ret.word64s, 256, data, str.length());  // size);
    return to_hex(ret);
}
}  // namespace qblocks
