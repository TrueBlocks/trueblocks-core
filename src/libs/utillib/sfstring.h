...
public:
	...
	long     findI           (const char *search) const;
	long     ReverseFind     (char ch) const;
	long     findExact       (const SFString& search, char sep, const SFString& replaceables=CHR_VALID_NAME) const;
	long     findExactI      (const SFString& search, char sep, const SFString& replaceables=CHR_VALID_NAME) const;
...
//--------------------------------------------------------------------
inline SFString asString(long i)
{
	char ret[128];
	sprintf(ret, "%ld", i);
	return SFString(ret);
...
//--------------------------------------------------------------------
extern void writeTheCode(const SFString& fileName, const SFString& code, bool spaces=true);

//-------------------------------------------------------------------------
inline long toLong(const char *str)    { return strtol(str,NULL,10); }
inline unsigned long toLongU(const char *str)   { return strtoul(str,NULL,10); }
