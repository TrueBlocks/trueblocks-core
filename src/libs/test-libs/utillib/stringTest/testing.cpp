...
TEST_F(CThisTest, TestCStr) {
    ...
    while ((token = strtok_r(rest, " ", &rest)))
        cout << TESTID("tokenize", 15) << padRight(token, 8, ' ') << "\t"
                    << (rest ? rest : "") << "\n";
    cout << "\n";

    string_q str1 = "Please|split|that|sentence|into|tokens";
    while (!str1.empty())
        cout << TESTID("nextTokClr", 15) << padRight(nextTokenClear(str1, '|'), 8, ' ') << "\t"
                    << str1 << "\n";
    cout << "\n";
    ...
    cout << TESTID("countOf", 15) << "There are " << countOf(strip2, 'B')
                << " 'Bs' in '|" << strip2 << "|'\n";
    cout << TESTID("countOf", 15) << "There are " << countOf(text2,  'i')
                << " 'is' in '" << substitute(text1, "\t", " ") << "'\n";
    cout << "\n";
