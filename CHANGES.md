...
- The changes in this release are in support of our docker version. The changes are focused on two tools (`serve` and `status`) which have been renamed to `daemon` and `config`, respectively. This change allows these tools to more clearly reflect what they do. A new tool called `chifra version` was also added.
