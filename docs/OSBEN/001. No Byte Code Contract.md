#### Problem:
	...
	Web3Scan reports 36 contract creations on 8/13/2015, QuickBlocks reports 35
...
Previously, QuickBlocks did not count this contract creation because it didn't look at traces of transactions that ended in error. QuickBlocks now counts this creation, but should it? Oddly, the contract [has no byte code](https://etherscan.io/address/0x1745096776ff53ac61baf5c6e6f569975a3aefb8#code).

#### Open Questions:
1. Should this be counted as a contract creation?
2. If the contract existed and returned, but then later got cleared because of 'out of gas' by caller, was it a true contract creation? In other words, is a `temporary contract` (on that doesn't persist past the end of the transaction) a contract?
3. If a contract has no byte code, should it be reported as a smart contract?
4. Is the creation of a contract considered changing state?
...
1. What happens if a newly created smart contract gets ether sent to it during construction and forwards the ether to either its creator or some other address. Then, later, the calling contract runs out of gas?
